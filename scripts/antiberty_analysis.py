...
os.makedirs('results/antiberty/plots', exist_ok=True)
...
# Load AntiBERTy model directly using the antiberty package
print("Loading AntiBERTy model...")
try:
    # Use AntiBERTyRunner as shown in the README
    from antiberty import AntiBERTyRunner
    
    # Initialize the model
    ab_model = AntiBERTyRunner()
    tokenizer = ab_model.tokenizer
    model = ab_model.model.to(device)
    model.eval()
    print("Successfully loaded AntiBERTy model")
    
    # Update the embedding function to use the Runner
    ...
    try:
        # Try direct initialization with config
        from antiberty import AntiBERTy
        from antiberty.config import AntiBERTyConfig
        
        config = AntiBERTyConfig()
        ab_model = AntiBERTy(config)
        tokenizer = ab_model.tokenizer
        model = ab_model.model.to(device)
        model.eval()
        print("Successfully loaded AntiBERTy with config")
        ...
        print("Could not initialize AntiBERTy. Please check installation.")
        exit(1)

# Process sequences to get embeddings
...
# Note: AntiBERTy is trained on antibody sequences, so might not perform
# optimally on viral sequences. Using it for RBD anyway for consistency.
print("Running inference for RBD...")
rbd_repr = get_antiberty_embeddings(rbd_seq)

print(f"Generated embeddings with shape: heavy {heavy_repr.shape}, light {light_repr.shape}, RBD {rbd_repr.shape}")

# Save embeddings
np.save('results/antiberty/heavy_chain_embeddings.npy', heavy_repr)
np.save('results/antiberty/light_chain_embeddings.npy', light_repr)
np.save('results/antiberty/rbd_embeddings.npy', rbd_repr)
...
np.save('results/antiberty/heavy_rbd_similarity.npy', heavy_rbd_sim)
np.save('results/antiberty/light_rbd_similarity.npy', light_rbd_sim)

# Visualize the similarity matrices
plt.figure(figsize=(12, 10))
plt.subplot(2, 1, 1)
sns.heatmap(heavy_rbd_sim, cmap='viridis')
plt.title('Heavy Chain-RBD Residue Similarity Matrix (AntiBERTy)')
plt.xlabel('RBD Residue Position')
plt.ylabel('Heavy Chain Residue Position')

plt.subplot(2, 1, 2)
sns.heatmap(light_rbd_sim, cmap='viridis')
plt.title('Light Chain-RBD Residue Similarity Matrix (AntiBERTy)')
plt.xlabel('RBD Residue Position')
plt.ylabel('Light Chain Residue Position')

plt.tight_layout()
plt.savefig('results/antiberty/plots/binding_similarity_heatmaps.png', dpi=300, bbox_inches='tight')
...
# Save results
with open('results/antiberty/binding_predictions.txt', 'w') as f:
...
plt.title('Heavy Chain Binding Scores (AntiBERTy)')
...
plt.title('Light Chain Binding Scores (AntiBERTy)')
plt.xlabel('Residue Position')
plt.ylabel('Binding Score')
...
plt.title('RBD Binding Scores (AntiBERTy)')
plt.xlabel('Residue Position')
plt.ylabel('Binding Score')

plt.tight_layout()
plt.savefig('results/antiberty/plots/binding_scores.png', dpi=300, bbox_inches='tight')

# Define approximate CDR regions based on typical Kabat numbering
heavy_cdrs = {
    'CDR-H1': range(26, 33),
    'CDR-H2': range(52, 58),  # Slightly adjusted based on your observations
    'CDR-H3': range(95, 103)
}
...
# Plot with CDR regions highlighted
...
plt.title('Heavy Chain Binding Scores with Approximate CDRs (AntiBERTy)')
plt.xlabel('Residue Position')
plt.ylabel('Binding Score')
plt.legend()
...
plt.title('Light Chain Binding Scores with Approximate CDRs (AntiBERTy)')
plt.xlabel('Residue Position')
plt.ylabel('Binding Score')
plt.legend()

plt.tight_layout()
plt.savefig('results/antiberty/plots/binding_scores_with_cdrs.png', dpi=300, bbox_inches='tight')
...
np.save('results/antiberty/heavy_rbd_energy.npy', heavy_rbd_energy)
np.save('results/antiberty/light_rbd_energy.npy', light_rbd_energy)
...
with open('results/antiberty/interface_residues.txt', 'w') as f:
...
# Save hotspots
with open('results/antiberty/binding_hotspots.txt', 'w') as f:
...
plt.axhline(y=heavy_binding_threshold, color='r', linestyle='--')
plt.title('Heavy Chain Binding Hotspots (AntiBERTy)')
plt.xlabel('Residue Position')
plt.ylabel('Binding Score')
plt.legend()
...
plt.axhline(y=light_binding_threshold, color='r', linestyle='--')
plt.title('Light Chain Binding Hotspots (AntiBERTy)')
plt.xlabel('Residue Position')
plt.ylabel('Binding Score')
plt.legend()
...
plt.axhline(y=rbd_binding_threshold, color='r', linestyle='--')
plt.title('RBD Binding Hotspots (AntiBERTy)')
plt.xlabel('Residue Position')
plt.ylabel('Binding Score')
plt.legend()

plt.tight_layout()
plt.savefig('results/antiberty/plots/binding_hotspots.png', dpi=300, bbox_inches='tight')
...
with open('results/antiberty/epitope_paratope_map.txt', 'w') as f:
...
with open('results/antiberty/top_interactions.txt', 'w') as f:
...
with open('results/antiberty/binding_energy_summary.txt', 'w') as f:
...
print("AntiBERTy analysis complete! All results saved to results/antiberty directory.")